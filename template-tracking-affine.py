...
def LK_parameterized_tracking(old_frame, template_box, new_frame, coord, params, affine_jacobian):
    ...
    for i in range(10):
        warped_Ix = cv2.warpAffine(Ix, params, dsize=(width, height), flags = (cv2.INTER_LINEAR + cv2.WARP_INVERSE_MAP))
        warped_Iy = cv2.warpAffine(Iy, params, dsize=(width, height), flags = (cv2.INTER_LINEAR + cv2.WARP_INVERSE_MAP))
        warped_I = cv2.warpAffine(I, params, dsize=(width, height), flags = (cv2.INTER_LINEAR + cv2.WARP_INVERSE_MAP))

        warped_grad_I = np.stack((warped_Ix, warped_Iy), axis = -1).reshape((height, width, 1, 2)) # height*width*1*2 dimension
        J = affine_jacobian                                                                         # height*width*2*num_params dimension
        ...
        delta_params = delta_params.reshape((2,3))   
    ...
    proj_params = np.append(params, [0, 0, 1])
...
affine_jacobian = getAffineJacobian(frames[0].shape[1], frames[0].shape[0])
for i in range(1, len(frames)):
    frame = frames[i]
    LK_track, params = LK_parameterized_tracking(frames[0], template_box, frame, coord, params, affine_jacobian)
